...
#		else{$cosmic=$a[29];}
		...
		my $k=join "\t",$a[14],$pos,$a[20],$a[5],$a[2],$a[3],$a[22],$a[23],$a[4],$a[12],$a[29],$a[48];
		my $o;
		if($a[8]=~/PASS/)
		{
			$o=$a[6]."|".$a[7];
...
print OT "Gene\tLocus\tExon\tGenotype\tRef\tAlt\tHGVsc\tHGVsp\tType\tConsequence\tExisting_variation\tClin_sig\t";
