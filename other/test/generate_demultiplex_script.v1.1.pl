...
foreach my $k(sort keys %hash){
	if($k eq "I6I0"){
		...
		print "/usr/local/bin/bcl2fastq -R $dir --minimum-trimmed-read-length 0 --mask-short-adapter-reads 0 --create-fastq-for-index-reads --no-lane-splitting --sample-sheet $hash{I8I8} 1>>demultiplex.sh.o 2>>demultiplex.sh.e\n";
	}
	elsif($k eq "I8UMI"){
		...
		print "/usr/local/bin/bcl2fastq -R $dir --minimum-trimmed-read-length 0 --mask-short-adapter-reads 0 --create-fastq-for-index-reads --no-lane-splitting --sample-sheet $hash{I8UMI} 1>>demultiplex.sh.o 2>>demultiplex.sh.e\n";}
	else{
		print "/usr/local/bin/bcl2fastq -R $dir --minimum-trimmed-read-length 0 --mask-short-adapter-reads 0 --create-fastq-for-index-reads --no-lane-splitting --sample-sheet $hash{$k} 1>>demultiplex.sh.o 2>>demultiplex.sh.e\n";
	}
}
#close OT;
