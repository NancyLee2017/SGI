...
while(<IN>){
	...
	if (($t[9]=~/A01D|D53D|H01D|G03D|D50D/)||($t[11]=~/ddCAP|Riscare58|OncoAim/)){#ddCAPonTissue，OncoAimDNA, CRC4货号；ddCAPV3货号; 甲状腺癌TC20货号； CRC22,HBOC19货号
		print OT "#\n";
		print OT "Name\t$t[7]\n";
		if($t[13]=~/^\s/){$t[13]="未提供";}
	...
	elsif(($t[9]=~/^(\s)*$/)&&($t[10]=~/甲状腺/)){#甲状腺癌27基因（ThyroidV2）
		print OT "Name\t$t[7]\n";
                if($t[13]=~/^\s/){$t[13]="未提供";}
                print OT "Gender\t$t[13]\n";
                if($t[12]=~/^\s/){$t[12]="未提供";}
