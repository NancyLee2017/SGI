#!/usr/bin/perl
use strict;
#分别输出OncoAimDNA的突变位点和QC,以及PGKB位点
#提供clinvar注释
...
open OT2,">QC.xls" or die "Error! Cannot creat QC file!\n";
open OT3,">PGKB.xls" or die "Error! Cannot creat PGKB file!\n";
...
#print OT2 "QC_info\n";#输出QC
print OT2 "SampleID\tTotal_read\tMapped_read\tMapped_rate\tOntarget_read\tOntarget_rate\tAverage_amplicon_coverage\tUniformity\n";
for(my $i=0;$i<=$#IDs;$i++){
	if(exists $qc{$IDs[$i]}){print OT2 "$IDs[$i]\t$qc{$IDs[$i]}\n";}
	else{print OT2 "$IDs[$i]\tQC_missing\n";}
}
...
#print OT "PGKB_info\n";#输出PGKB
print OT3 "SampleID\tGeneName\tLocusName\tPosition\tRef\tAlt\tGenotype\tPGKB_type\tcDNA_change\tProtein_change\tFrequency\tDepth\n";

for(my $i=0;$i<=$#IDs;$i++){
	foreach my $sk(sort keys %key){
			print OT3 "$IDs[$i]\t$sk\t";
			if(exists $hash{$IDs[$i]}{$sk}){print OT3 "$hash{$IDs[$i]}{$sk}\n";}
			else{print OT3 "NA\n";}
		}
}
