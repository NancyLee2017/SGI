#!/usr/bin/perl -w
use strict;
die "perl $0 <in.all.tsv.list> <need_to_remove.pos.list> <client,eg: sgi> <out.generate_pdf.sh> <server:e.g:CLS1 or Mars>\n" unless (@ARGV ==5);
...
elsif($ARGV[4]=~/CLS1/i){$generate_pdf="/data/home/jiecui/software/git/ctdna-pipeline/scripts/CreateVariantReport/new_oncoaim/oncoaim_tsv_analysis.php";}
...
#	$pdf=~s/\.report.filtered.tsv/\_report.$ARGV[3].pdf/;
	...
	elsif($f[1]=~/ddCAPonTissu/i){$template="oncoaim_lung_CN";$fusion=$fusion1;}
	elsif($f[1]=~/ddCAP(\_?)v2/i){$template="plasaim_lung_CN"; $fusion=$fusion2;}
	elsif($f[1]=~/OncoAim/i){$template="oncoaim_CN";}
	else{print "No template\n";}
	$pdf=~s/\.report.filtered.tsv/\_report.$template.pdf/;
	if(-e $cnv){
		print OTS "php $generate_pdf $f[1] $ARGV[2] $pdf $json $template $protsv $fusion $cnv\n";}
	else{
		print OTS "php $generate_pdf $f[1] $ARGV[2] $pdf $json $template $protsv\n";
	}
}close LST;
close OTS;
