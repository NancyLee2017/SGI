...
die "perl $0 <uniq_site.list> <raw_matrix.list> <combined_matrix.xls>\nuniq_site format: BRCA1\tchr17:41223094\t15\tT\tC
\n" unless (@ARGV ==3);
...
#			}
	...
	foreach my $k (sort keys %site){
		...
		else {my $x=$length-5; my $fill="\t"x$x; $hash{$k}=$hash{$k}.$k."\t".$fill."\t\t";}
	}
